...
- [x] 処理済みトグル（ツールバーの「済」ボタンで現在ファイル名の先頭に【済】を付け外し）👍
  - File System Access の move() でリネーム。再押下でトグル解除
- [x] フォルダ再読み込み（各フォルダの↻ボタンでツリーだけ再走査・新規ファイルを反映）👍
  - 保持済みハンドルで build_tree を再実行。ページ全体のリロード不要
- [x] 【済】ファイルの表示フィルタ（サイドバーのトグルで非表示/表示・localStorage保持）👍
  - filter_out_prefixed で表示用に除外（実ファイルは変更しない）。空になったフォルダも非表示
- [x] 外部変更の自動検知（開いているファイルの更新時刻をポーリング）👍
  - 未編集なら自動再読み込み、未保存編集中は上書きせずバナー通知＋再読み込みボタン
  - File System Access に監視APIが無いため lastModified ポーリング（3秒）。保存/リネーム時はmtimeを取り直し誤検知を防止
- [x] 初回アクセス時のチュートリアルツアー自動表示（各ボタンを順番に吹き出しで案内・?ボタンで再生・localStorage で表示済み管理）👍
  - lite-md:help-seen フラグで初回のみ自動開始。スポットライト＋吹き出しを自前実装（外部依存なし）。対象が非表示のステップは自動スキップ
- [x] タスクリスト対応（- [ ] / - [x] をチェックボックス描画・プレビューのクリックでソースの [ ]/[x] を切替）👍
  - markdown-it プラグインを自前実装（依存追加なし）。data-line のソース行番号で対象行を特定し、タスク行であることを検証してから反転（編集ずれの誤反転防止）。切替は通常の編集扱い（手動保存なら Ctrl+S）
